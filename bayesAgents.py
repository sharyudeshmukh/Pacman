...
def constructBayesNet(gameState):
    ...
    # everything adding to the dictionary
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    loc = gameState.getPossibleHouses()
    for housePos in loc:
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for r in obsCPT:
                ghostHouseVal = r[GHOST_HOUSE_VAR]
                foodHouseVal = r[FOOD_HOUSE_VAR]
                c = r[obsVar]
                ...
                if quadrant != ghostHouseVal and quadrant != foodHouseVal:
                    ...
                    if c == RED_OBS_VAL:
                        ...
                        p = 0
                obsVarFactor.setProbability(r, p)
...
def getAdjQuadrant(gameState, obsPos):
    ...
    for h in gameState.getPossibleHouses():
        x = util.manhattanDistance(obsPos, h)
        if x < d:
            d = x
            house_neighbor = h

    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
    if house_neighbor == bottomLeftPos:
        return BOTTOM_LEFT_VAL
    if house_neighbor == topLeftPos:
        return TOP_LEFT_VAL
    if house_neighbor == bottomRightPos:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    fac_query = inferenceByVariableElimination(bayesNet, ['foodHouse', 'ghostHouse'], evidence, eliminationOrder)
    highest_assignment_dict = dict()
    p = 0
    for a in fac_query.getAllPossibleAssignmentDicts():
        p_temp = fac_query.getProbability(a)
        if p_temp > p:
            p = p_temp
            highest_assignment_dict = a
    return highest_assignment_dict
    #util.raiseNotDefined()
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        from inference import inferenceByVariableElimination
        fac_query = inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        
        leftfood_prob, rightfood_prob = 0, 0
        for a in fac_query.getAllPossibleAssignmentDicts():
            if a[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and a[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftfood_prob = fac_query.getProbability(a)
            elif a[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and a[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightfood_prob = fac_query.getProbability(a)

        leftExpectedValue = leftfood_prob * WON_GAME_REWARD + (1  - leftfood_prob) * GHOST_COLLISION_REWARD
        rightExpectedValue = rightfood_prob * WON_GAME_REWARD + (1 - rightfood_prob) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for (p, evi) in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue = expectedValue + p * max(self.computeEnterValues(evi, enterEliminationOrder))
